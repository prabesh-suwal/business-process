...
public class WorkflowAdminController {
        ...
        /**
         * Get all users for a product (for specific user assignment).
         */
        @GetMapping("/users")
        public ResponseEntity<List<DropdownItem>> getUsers(
                        @RequestParam(defaultValue = "MMS") String productCode) {
                var users = adminService.listUsersByProduct(productCode);
        ...
        public ResponseEntity<List<DropdownItem>> getScopes() {
                var scopes = List.of(
                                DropdownItem.builder().id("INITIATOR_BRANCH").code("INITIATOR_BRANCH")
                                                .label("Same Branch as Initiator")
                                                .build(),
                                DropdownItem.builder().id("CUSTOMER_BRANCH").code("CUSTOMER_BRANCH")
                                                .label("Customer's Branch").build(),
                                DropdownItem.builder().id("SAME_DEPARTMENT").code("SAME_DEPARTMENT")
                                                .label("Same Department").build(),
                                ...
                                DropdownItem.builder().id("HEAD_OFFICE").code("HEAD_OFFICE").label("Head Office")
                                                .build(),
        ...
        public ResponseEntity<List<DropdownItem>> getAssignmentTypes() {
                var types = List.of(
                                ...
                                DropdownItem.builder().id("SPECIFIC_USER").code("SPECIFIC_USER").label("Specific User")
                                                .build(),
                                DropdownItem.builder().id("RULE_BASED").code("RULE_BASED")
                                                .label("Rule-based (Advanced)").build());
        ...
        public ResponseEntity<List<OperatorItem>> getOperators() {
                var operators = List.of(
                                OperatorItem.builder().id("==").label("equals")
                                                .types(List.of("STRING", "NUMBER", "BOOLEAN")).build(),
                                OperatorItem.builder().id("!=").label("not equals").types(List.of("STRING", "NUMBER"))
                                                .build(),
                                OperatorItem.builder().id(">").label("greater than").types(List.of("NUMBER")).build(),
                                OperatorItem.builder().id(">=").label("greater or equal").types(List.of("NUMBER"))
                                                .build(),
                                ...
                                OperatorItem.builder().id("contains").label("contains").types(List.of("STRING"))
                                                .build(),
                                OperatorItem.builder().id("startsWith").label("starts with").types(List.of("STRING"))
                                                .build());
        ...
        public ResponseEntity<List<DropdownItem>> getEscalationActions() {
                var actions = List.of(
                                ...
                                DropdownItem.builder().id("ADD_APPROVER").code("ADD_APPROVER")
                                                .label("Add Parallel Approver").build(),
                                DropdownItem.builder().id("ESCALATE_TO_MANAGER").code("ESCALATE_TO_MANAGER")
                                                .label("Escalate to Manager").build(),
                                DropdownItem.builder().id("AUTO_COMPLETE").code("AUTO_COMPLETE").label("Auto-Complete")
                                                .build());
