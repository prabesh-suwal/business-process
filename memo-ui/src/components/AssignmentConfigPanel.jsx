import React, { useState, useEffect } from 'react';
import { X, Check, ChevronDown, User, Users, Building2 } from 'lucide-react';
import { CasAdminApi } from '../lib/api';

/**
 * AssignmentConfigPanel - Enterprise-level multi-select UI for workflow step assignment
 * Features:
 * - Dropdown-based multi-selection (no typing required)
 * - Support for selecting multiple roles, departments, and users
 * - Visual chips/tags for selected items
 * - Non-technical, user-friendly language
 */
const AssignmentConfigPanel = ({
    config = {},
    onChange,
    title = "Who should handle this step?"
}) => {
    // Dropdown data from backend
    const [roles, setRoles] = useState([]);
    const [departments, setDepartments] = useState([]);
    const [users, setUsers] = useState([]);
    const [loading, setLoading] = useState(true);

    // Extract selected items from config
    const selectedRoles = config.roles || [];
    const selectedDepartments = config.departments || [];
    const selectedUsers = config.users || [];

    // Dropdown open states
    const [roleDropdownOpen, setRoleDropdownOpen] = useState(false);
    const [deptDropdownOpen, setDeptDropdownOpen] = useState(false);
    const [userDropdownOpen, setUserDropdownOpen] = useState(false);

    useEffect(() => {
        loadDropdownData();
    }, []);

    const loadDropdownData = async () => {
        try {
            setLoading(true);
            const [rolesData, deptsData, usersData] = await Promise.all([
                CasAdminApi.getRoles().catch(() => []),
                CasAdminApi.getDepartments().catch(() => []),
                CasAdminApi.getUsers().catch(() => [])
            ]);

            // Fallback data if API returns empty
            setRoles(rolesData.length > 0 ? rolesData : [
                { id: '1', code: 'MMS_REVIEWER', name: 'Reviewer', label: 'Reviewer' },
                { id: '2', code: 'MMS_APPROVER', name: 'Approver', label: 'Approver' },
                { id: '3', code: 'MMS_MANAGER', name: 'Manager', label: 'Manager' },
                { id: '4', code: 'MMS_DIRECTOR', name: 'Director', label: 'Director' },
                { id: '5', code: 'MMS_EXECUTIVE', name: 'Executive', label: 'Executive' }
            ]);

            setDepartments(deptsData.length > 0 ? deptsData : [
                { id: '1', code: 'FINANCE', name: 'Finance Department' },
                { id: '2', code: 'HR', name: 'Human Resources' },
                { id: '3', code: 'LEGAL', name: 'Legal Department' },
                { id: '4', code: 'AUDIT', name: 'Internal Audit' },
                { id: '5', code: 'OPERATIONS', name: 'Operations' },
                { id: '6', code: 'RISK', name: 'Risk Management' }
            ]);

            setUsers(usersData.length > 0 ? usersData : [
                { id: 'u1', userId: 'admin-001', username: 'Admin User', email: 'admin@company.com' },
                { id: 'u2', userId: 'manager-001', username: 'John Manager', email: 'john@company.com' },
                { id: 'u3', userId: 'director-001', username: 'Sarah Director', email: 'sarah@company.com' },
                { id: 'u4', userId: 'ceo-001', username: 'CEO', email: 'ceo@company.com' }
            ]);
        } catch (error) {
            console.error('Failed to load dropdown data:', error);
        } finally {
            setLoading(false);
        }
    };

    // Toggle role selection
    const toggleRole = (roleCode) => {
        const isSelected = selectedRoles.includes(roleCode);
        const newRoles = isSelected
            ? selectedRoles.filter(r => r !== roleCode)
            : [...selectedRoles, roleCode];

        onChange({ ...config, roles: newRoles });
    };

    // Toggle department selection
    const toggleDepartment = (deptCode) => {
        const isSelected = selectedDepartments.includes(deptCode);
        const newDepts = isSelected
            ? selectedDepartments.filter(d => d !== deptCode)
            : [...selectedDepartments, deptCode];

        onChange({ ...config, departments: newDepts });
    };

    // Toggle user selection
    const toggleUser = (userId) => {
        const isSelected = selectedUsers.includes(userId);
        const newUsers = isSelected
            ? selectedUsers.filter(u => u !== userId)
            : [...selectedUsers, userId];

        onChange({ ...config, users: newUsers });
    };

    // Remove item by type
    const removeItem = (type, value) => {
        if (type === 'role') {
            onChange({ ...config, roles: selectedRoles.filter(r => r !== value) });
        } else if (type === 'department') {
            onChange({ ...config, departments: selectedDepartments.filter(d => d !== value) });
        } else if (type === 'user') {
            onChange({ ...config, users: selectedUsers.filter(u => u !== value) });
        }
    };

    // Get display name
    const getDisplayName = (type, value) => {
        if (type === 'role') {
            const role = roles.find(r => r.code === value);
            return role ? role.label || role.name : value;
        }
        if (type === 'department') {
            const dept = departments.find(d => d.code === value);
            return dept ? dept.name : value;
        }
        if (type === 'user') {
            const user = users.find(u => (u.id || u.userId) === value);
            return user ? user.label || user.username || user.code : value;
        }
        return value;
    };

    // Get color for chip
    const getChipColor = (type) => {
        switch (type) {
            case 'role': return 'bg-blue-100 text-blue-800 border-blue-200';
            case 'department': return 'bg-green-100 text-green-800 border-green-200';
            case 'user': return 'bg-purple-100 text-purple-800 border-purple-200';
            default: return 'bg-gray-100 text-gray-800 border-gray-200';
        }
    };

    // Get icon for type
    const getIcon = (type) => {
        switch (type) {
            case 'role': return <Users className="w-3 h-3" />;
            case 'department': return <Building2 className="w-3 h-3" />;
            case 'user': return <User className="w-3 h-3" />;
            default: return <Users className="w-3 h-3" />;
        }
    };

    // Count total selections
    const totalSelections = selectedRoles.length + selectedDepartments.length + selectedUsers.length;

    if (loading) {
        return (
            <div className="space-y-3">
                <div className="animate-pulse h-10 bg-gray-200 rounded"></div>
                <div className="animate-pulse h-10 bg-gray-200 rounded"></div>
            </div>
        );
    }

    return (
        <div className="space-y-4">
            {title && (
                <div className="flex items-center justify-between">
                    <h3 className="text-sm font-medium text-gray-700">{title}</h3>
                    {totalSelections > 0 && (
                        <span className="text-xs text-gray-500">{totalSelections} selected</span>
                    )}
                </div>
            )}

            {/* Selected Items - Visual Chips */}
            {totalSelections > 0 && (
                <div className="flex flex-wrap gap-2 p-3 bg-gray-50 rounded-lg border">
                    {selectedRoles.map(roleCode => (
                        <span
                            key={`role-${roleCode}`}
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getChipColor('role')}`}
                        >
                            {getIcon('role')}
                            <span>{getDisplayName('role', roleCode)}</span>
                            <button
                                onClick={() => removeItem('role', roleCode)}
                                className="ml-1 hover:bg-white/50 rounded-full p-0.5"
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </span>
                    ))}
                    {selectedDepartments.map(deptCode => (
                        <span
                            key={`dept-${deptCode}`}
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getChipColor('department')}`}
                        >
                            {getIcon('department')}
                            <span>{getDisplayName('department', deptCode)}</span>
                            <button
                                onClick={() => removeItem('department', deptCode)}
                                className="ml-1 hover:bg-white/50 rounded-full p-0.5"
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </span>
                    ))}
                    {selectedUsers.map(userId => (
                        <span
                            key={`user-${userId}`}
                            className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${getChipColor('user')}`}
                        >
                            {getIcon('user')}
                            <span>{getDisplayName('user', userId)}</span>
                            <button
                                onClick={() => removeItem('user', userId)}
                                className="ml-1 hover:bg-white/50 rounded-full p-0.5"
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </span>
                    ))}
                </div>
            )}

            {/* Selection Dropdowns */}
            <div className="space-y-3">
                {/* Roles Dropdown */}
                <div className="relative">
                    <button
                        type="button"
                        onClick={() => {
                            setRoleDropdownOpen(!roleDropdownOpen);
                            setDeptDropdownOpen(false);
                            setUserDropdownOpen(false);
                        }}
                        className="w-full flex items-center justify-between p-3 bg-white border rounded-lg hover:bg-gray-50 transition-colors"
                    >
                        <div className="flex items-center gap-2">
                            <Users className="w-4 h-4 text-blue-600" />
                            <span className="text-sm font-medium">By Role</span>
                            {selectedRoles.length > 0 && (
                                <span className="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded-full">
                                    {selectedRoles.length} selected
                                </span>
                            )}
                        </div>
                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${roleDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {roleDropdownOpen && (
                        <div className="absolute z-20 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            {roles.map(role => (
                                <button
                                    key={role.code}
                                    type="button"
                                    onClick={() => toggleRole(role.code)}
                                    className="w-full flex items-center justify-between p-3 hover:bg-blue-50 transition-colors text-left"
                                >
                                    <span className="text-sm">{role.label || role.name}</span>
                                    {selectedRoles.includes(role.code) && (
                                        <Check className="w-4 h-4 text-blue-600" />
                                    )}
                                </button>
                            ))}
                        </div>
                    )}
                </div>

                {/* Departments Dropdown */}
                <div className="relative">
                    <button
                        type="button"
                        onClick={() => {
                            setDeptDropdownOpen(!deptDropdownOpen);
                            setRoleDropdownOpen(false);
                            setUserDropdownOpen(false);
                        }}
                        className="w-full flex items-center justify-between p-3 bg-white border rounded-lg hover:bg-gray-50 transition-colors"
                    >
                        <div className="flex items-center gap-2">
                            <Building2 className="w-4 h-4 text-green-600" />
                            <span className="text-sm font-medium">By Department</span>
                            {selectedDepartments.length > 0 && (
                                <span className="bg-green-100 text-green-700 text-xs px-2 py-0.5 rounded-full">
                                    {selectedDepartments.length} selected
                                </span>
                            )}
                        </div>
                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${deptDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {deptDropdownOpen && (
                        <div className="absolute z-20 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            {departments.map(dept => (
                                <button
                                    key={dept.code}
                                    type="button"
                                    onClick={() => toggleDepartment(dept.code)}
                                    className="w-full flex items-center justify-between p-3 hover:bg-green-50 transition-colors text-left"
                                >
                                    <span className="text-sm">{dept.name}</span>
                                    {selectedDepartments.includes(dept.code) && (
                                        <Check className="w-4 h-4 text-green-600" />
                                    )}
                                </button>
                            ))}
                        </div>
                    )}
                </div>

                {/* Specific Users Dropdown */}
                <div className="relative">
                    <button
                        type="button"
                        onClick={() => {
                            setUserDropdownOpen(!userDropdownOpen);
                            setRoleDropdownOpen(false);
                            setDeptDropdownOpen(false);
                        }}
                        className="w-full flex items-center justify-between p-3 bg-white border rounded-lg hover:bg-gray-50 transition-colors"
                    >
                        <div className="flex items-center gap-2">
                            <User className="w-4 h-4 text-purple-600" />
                            <span className="text-sm font-medium">Specific People</span>
                            {selectedUsers.length > 0 && (
                                <span className="bg-purple-100 text-purple-700 text-xs px-2 py-0.5 rounded-full">
                                    {selectedUsers.length} selected
                                </span>
                            )}
                        </div>
                        <ChevronDown className={`w-4 h-4 text-gray-400 transition-transform ${userDropdownOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {userDropdownOpen && (
                        <div className="absolute z-20 w-full mt-1 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            {users.map((user, idx) => (
                                <button
                                    key={user.id || user.userId || user.code || idx}
                                    type="button"
                                    onClick={() => toggleUser(user.id || user.userId)}
                                    className="w-full flex items-center justify-between p-3 hover:bg-purple-50 transition-colors text-left"
                                >
                                    <div className="flex flex-col">
                                        <span className="text-sm font-medium">{user.label || user.username || user.code}</span>
                                        <span className="text-xs text-gray-500">{user.code || user.email}</span>
                                    </div>
                                    {selectedUsers.includes(user.id || user.userId) && (
                                        <Check className="w-4 h-4 text-purple-600" />
                                    )}
                                </button>
                            ))}
                        </div>
                    )}
                </div>
            </div>

            {/* Empty State */}
            {totalSelections === 0 && (
                <div className="text-center py-3 text-xs text-gray-500">
                    Select who should handle tasks in this step
                </div>
            )}
        </div>
    );
};

export default AssignmentConfigPanel;
